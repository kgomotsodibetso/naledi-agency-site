...
export function Contact() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (result.success) {
            ...
            setStatus('error');
            setServerMessage(result.message || 'An unexpected error occurred.');
            if (result.errors) {
                    ...
                    {status === 'success' ? (
                        ...
                        <>
                            <form onSubmit={handleSubmit} noValidate className="space-y-4">
                                ...
                                <div>
                                    <label htmlFor="service" className="block text-sm font-bold text-slate-700 mb-1">What do you need?</label>
                                    <select id="service" value={formData.service} onChange={handleChange} className="w-full px-4 py-2 border border-slate-300 rounded-md bg-slate-100 text-slate-900 focus:ring-sunrise-yellow focus:border-sunrise-yellow">
                                ...
                                {status === 'error' && serverMessage && !errors.name && !errors.email && !errors.message && (
